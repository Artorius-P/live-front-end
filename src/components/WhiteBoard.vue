<!--
<template>
    <iframe :src="`http://127.0.0.1:5000/whiteboard`"  width="1200px" height="800px"></iframe>
</template>
-->
<template>
  ...
  methods: {
    ...
    updateModifications(savehistory) {
      if (savehistory == true) {
        ...
        //console.log(this.fabricHistoryJson);
        socket.emit("wserver", {
          room: "white" + this.$store.state.room.id,
          history: JSON.stringify(this.fabricObj),
        });
      }
    },
    //canvas 历史后退
